...
def change_superconductors(Igoal,readvars):
        ...
        def check_done(done,Inow,Igoal,Ioff):
            ...
            if done[3]==0 and direction[3]*(Inow[3]-Ioff[3])>0:
                ...
                print('sext to goal:',done,' Inow:',Inow[3],' Igoal:',Igoal[3])
            return(done)
...
#                    int(done[2]),Inow[2],Igoal[2],Ioff[2],np.sum(checkdone[2,:])/diffall))
            ...
            if time.time()-time_start_change >300.0:
                ...
                print('got stuck at ',venus.read(['inj_i']),venus.read(['ext_i']),
                        venus.read(['mid_i']), venus.read(['sext_i']))   # current currents
                Inow=np.array([venus.read(['inj_i']),venus.read(['ext_i']),
                    venus.read(['mid_i']),venus.read(['sext_i'])])   # current currents
                for i in range(4):
                    print('in here. i=%i, Inow[i]=%f, Igoal[i]=%f, done[i]=%i'
                            %(i,Inow[i],Igoal[i],done[i]))
...
coil = 3 #0:inj,1:ext,2:mid
coilname = ['inj','ext','mid','sext']
base = np.array([185.,137.,153.,430.0])
currentchange = 5.0
